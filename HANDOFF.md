...
- URL: server.arcgisonline.com/.../World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}
- 匿名访问无 key、亮度 42 vs CARTO 35(视觉接近)、支持到 z20(CARTO 是 19)
- attribution 已按 Esri 要求更新("Tiles © Esri — Esri, DeLorme, NAVTEQ | © OSM contributors"),CARTO credit 移除
- 想换回 CARTO:initMap 注释里有说明(申请免费 key 加 ?key=)
- 实测:16/16 tiles 加载 0 broken,attribution 正确,marker 不受影响
