...
// Pure display/geometry helpers live in their own module so they can be unit
// tested without mounting the component (see tests/geo.test.mjs).
import {
  computeWaveStatus,
  dedupeEvents,
  estimatedIntensity,
...
watch([selectedEvent, userLocation], ([eq]) => {
  waveStatus.value = locateEnabled.value ? computeWaveStatus(eq, userLocation.value) : null
}, { immediate: false })
...
async function loadShakeMap(eq) {
  ...
  try {
    const response = await fetchWithTimeout(eq.detail, { timeoutMs: 12_000 })
...
async function fetchCatalogEvents(feed, sourceKeys = selectedSources.value) {
  ...
  // events while still being listed as active. Each source gets a couple of
  // backed-off retries so a transient blip doesn't blank it for a full cycle.
  const health = { usgs: 'idle', emsc: 'idle', gfz: 'idle', geonet: 'idle' }
  const tasks = []
  const addTask = (key, promiseFactory) => {
    tasks.push(withRetry(promiseFactory)
      .then(items => {
      ...
      .catch(err => {
        health[key] = 'error'
        console.warn(`${key.toUpperCase()} source failed after retries:`, err)
        return []
      }))
  }
